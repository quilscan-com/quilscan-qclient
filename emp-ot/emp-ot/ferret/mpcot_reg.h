...
public:
  int64_t party, threads;
	...
	MpcotReg(int64_t party, int64_t threads, int64_t n, int64_t t, int64_t log_bin_sz, ThreadPool * pool, IO **ios) {
		this->party = party;
	...
	void exec_parallel_sender(vector<SPCOT_Sender<IO>*> &senders,
		...
		int64_t width = tree_n / threads;
		int64_t start = 0, end = width;
	...
	void exec_f2k_sender(SPCOT_Sender<IO> *sender, OTPre<IO> *ot, 
			block *ggm_tree_mem, IO *io, int64_t i) {
	...
	void consistency_check_f2k(block *pre_cot_data, int64_t num) {
		if(this->party == ALICE) {
			block r1, r2;
