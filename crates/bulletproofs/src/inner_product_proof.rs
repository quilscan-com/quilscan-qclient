...
pub struct InnerProductProof {
    pub(crate) L_vec: Vec<CompressedPoint>,
    pub(crate) R_vec: Vec<CompressedPoint>,
    pub(crate) a: Scalar,
    pub(crate) b: Scalar,
}
...
impl InnerProductProof {
    ...
    pub fn create(
        ...
        if n != 1 {
            ...
            for i in 0..n {
                ...
                G_L[i] = Point::vartime_multiscalar_mul(
                    &[u_inv * G_factors[i], u * G_factors[n + i]],
                    &[G_L[i], G_R[i]],
                );
                H_L[i] = Point::vartime_multiscalar_mul(
        ...
        while n != 1 {
            ...
            for i in 0..n {
                ...
                H_L[i] = Point::vartime_multiscalar_mul(&[u, u_inv], &[H_L[i], H_R[i]]);
            }
        ...
        IH::Item: Borrow<Scalar>,
    {
        ...
        let expect_P = Point::vartime_multiscalar_mul(
    ...
    /// \\(56 \cdot (2\lg n+2)\\) bytes for Decaf (versus 32 bytes per element for Ed25519).
    pub fn serialized_size(&self) -> usize {
        // Decaf uses 56-byte compressed points and 56-byte scalars, but we zero byte prefix the scalars for simpler serialization
        (self.L_vec.len() * 2 + 2) * 56
    }

    /// Serializes the proof into a byte array of \\(2n+2\\) 56-byte elements.
    ...
    /// * the slice does not have \\(2n+2\\) 56-byte elements for Decaf,
    /// * \\(n\\) is larger or equal to 56 (proof is too big),
    /// * any of \\(2n\\) points are not valid compressed Ed448 points,
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<InnerProductProof, ProofError> {
        ...
        let mut L_vec: Vec<CompressedPoint> = Vec::with_capacity(lg_n);
        let mut R_vec: Vec<CompressedPoint> = Vec::with_capacity(lg_n);
        for i in 0..lg_n {
            let pos = 2 * i * 56;

            // Create CompressedPoint from the byte slice
            let l_bytes = &slice[pos..pos + 56];
            let r_bytes = &slice[pos + 56..pos + 112];

            // Create compressed points using the adapter
            L_vec.push(CompressedPoint::from_slice(l_bytes));
            R_vec.push(CompressedPoint::from_slice(r_bytes));
        }
...
mod tests {
    ...
    fn test_helper_create(n: usize) {
        ...
        let G: Vec<Point> = bp_gens.share(0).G(n).cloned().collect();
        let H: Vec<Point> = bp_gens.share(0).H(n).cloned().collect();

        // Q would be determined upstream in the protocol, so we pick a random one.
        let Q = Point::hash_from_bytes::<Shake256>(b"test point");
        ...
        let P = Point::vartime_multiscalar_mul(
