...
use crate::curve_adapter::Scalar;
use alloc::vec;
use alloc::vec::Vec;
...
#[cfg(all(feature = "yoloproofs", feature = "clear_on_drop"))]
...
/// Given `data` with `len >= 56`, return the first 56 bytes.
pub fn read56(data: &[u8]) -> [u8; 56] {
    let mut buf56 = [0u8; 56];
    buf56[..].copy_from_slice(&data[..56]);
    buf56
}
...
mod tests {
    ...
    #[test]
    fn test_scalar_exp() {
        // Use a simple scalar for testing with ed448
    ...
    fn vec_of_scalars_clear_on_drop() {
        ...
        assert_eq!(flat_slice(&v.as_slice()), &[0u8; 112][..]);
