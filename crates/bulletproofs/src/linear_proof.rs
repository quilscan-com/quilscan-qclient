...
impl LinearProof {
    ...
    pub fn create<T: RngCore + CryptoRng>(
        ...
        while n != 1 {
            ...
            for i in 0..n {
                ...
                // G_L = G_L + x_j * G_R
                G_L[i] = Point::vartime_multiscalar_mul(&[Scalar::one(), x_j], &[G_L[i], G_R[i]]);
            }
            a = a_L;
            b = b_L;
            G = G_L;
    ...
    pub fn verify(
        ...
        let L_R_factors: Point = Point::vartime_multiscalar_mul(
        ...
        let G_0: Point = Point::vartime_multiscalar_mul(s.iter(), G.iter());
    ...
    /// \\(56 \cdot (2\lg n+3)\\) bytes.
    pub fn serialized_size(&self) -> usize {
        (self.L_vec.len() * 2 + 3) * 56
    }

    /// Serializes the proof into a byte array of \\(2n+3\\) 56-byte elements.
    ...
    /// * the slice does not have \\(2n+3\\) 56-byte elements,
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<LinearProof, ProofError> {
        ...
        use crate::util::read56;

        let mut L_vec: Vec<CompressedPoint> = Vec::with_capacity(lg_n);
        let mut R_vec: Vec<CompressedPoint> = Vec::with_capacity(lg_n);
        for i in 0..lg_n {
            let pos = 2 * i * 56;
            L_vec.push(CompressedPoint::from_slice(&slice[pos..pos + 56]));
            R_vec.push(CompressedPoint::from_slice(&slice[pos + 56..pos + 112]));
        }

        let pos = 2 * lg_n * 56;
        let S = CompressedPoint::from_slice(&slice[pos..pos + 56]);
        let a = Scalar::from_canonical_bytes(read56(&slice[pos + 56..]))
            .ok_or(ProofError::FormatError)?;
        let r = Scalar::from_canonical_bytes(read56(&slice[pos + 112..]))
            .ok_or(ProofError::FormatError)?;
...
mod tests {
    ...
    fn test_helper(n: usize) {
        ...
        let C = Point::vartime_multiscalar_mul(
