...
pub struct PolyCommitment {
    pub(super) T_1_j: Point,
    pub(super) T_2_j: Point,
}
...
impl ProofShare {
    ...
    pub(super) fn audit_share(
        ...
        let P_check = Point::vartime_multiscalar_mul(
        ...
        let t_check = Point::vartime_multiscalar_mul(
            iter::once(zz * z_j)
                .chain(iter::once(*x))
                .chain(iter::once(x * x))
