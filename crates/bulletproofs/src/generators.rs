...
// Import our curve adapter types
use crate::curve_adapter::{Point, Scalar, BASEPOINT_COMPRESSED, BASEPOINT_POINT};
...
///
/// * `B`: the `ed448` basepoint;
/// * `B_blinding`: the result of `ed448` SHA3-512
...
pub struct PedersenGens {
    ...
    pub B_blinding: Point,
}

impl PedersenGens {
    /// Creates a Pedersen commitment using the value scalar and a blinding factor.
    pub fn commit(&self, value: Scalar, blinding: Scalar) -> Point {
        Point::multiscalar_mul(&[value, blinding], &[self.B, self.B_blinding])
    }
}
...
impl Default for PedersenGens {
    fn default() -> Self {
        PedersenGens {
            B: *BASEPOINT_POINT,
            B_blinding: Point::hash_from_bytes::<sha3::Shake256>(BASEPOINT_COMPRESSED.as_bytes()),
        }
    }
}
...
impl Iterator for GeneratorsChain {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(Point::from_uniform_bytes(&uniform_bytes))
    }
...
/// XOF output into the `ed448` hash-to-group function.
...
impl BulletproofGens {
    ...
    pub(crate) fn G(&self, n: usize, m: usize) -> impl Iterator<Item = Point> + '_ {
        AggregatedGensIter {
            n,
            m,
            array: &self.G_vec,
    ...
    pub(crate) fn H(&self, n: usize, m: usize) -> impl Iterator<Item = Point> + '_ {
        AggregatedGensIter {
            n,
            m,
            array: &self.H_vec,
...
mod tests {
    ...
    fn aggregated_gens_iter_matches_flat_map() {
        ...
        let helper = |n: usize, m: usize| {
            let agg_G: Vec<Point> = gens.G(n, m).collect();
            let flat_G: Vec<Point> = gens
                .G_vec
                .iter()
                .take(m)
            ...
            let agg_H: Vec<Point> = gens.H(n, m).collect();
            let flat_H: Vec<Point> = gens
                .H_vec
                .iter()
                .take(m)
    ...
    fn resizing_small_gens_matches_creating_bigger_gens() {
        ...
        let helper = |n: usize, m: usize| {
            let gens_G: Vec<Point> = gens.G(n, m).collect();
            let gens_H: Vec<Point> = gens.H(n, m).collect();

            let resized_G: Vec<Point> = gen_resized.G(n, m).collect();
            let resized_H: Vec<Point> = gen_resized.H(n, m).collect();
