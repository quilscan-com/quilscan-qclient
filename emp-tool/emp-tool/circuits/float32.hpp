inline Float::Float(float input, int64_t party) {
	int *in = (int*)(&input);
...
template<>
inline string Float::reveal<string>(int64_t party) const {
	int out = 0;
...
template<>
inline double Float::reveal<double>(int64_t party) const {
	int out = 0;
	for(int i = 0; i < FLOAT_LEN; ++i) {	
