...
inline void Integer::init(bool * b, int len, int64_t party) {
	bits.resize(len);
	if (party == PUBLIC) {
...
inline Integer::Integer(int len, int64_t input, int64_t party) {
	bool* b = new bool[len];
...
template<typename T>
inline Integer::Integer(int len, T * input, int64_t party) {
	bool* b = new bool[len];
...
template<typename T>
inline Integer::Integer(T * input, int64_t party) {
...
inline void Integer::revealBools(bool *bools, int64_t party) const {
	ProtocolExecution::prot_exec->reveal(bools, party, (block *)bits.data(), size());
}

template<>
inline uint32_t Integer::reveal<uint32_t>(int64_t party) const {
	std::bitset<32> bs;
	bs.reset();
	bool b[size()];
...
template<>
inline uint64_t Integer::reveal<uint64_t>(int64_t party) const {
	std::bitset<64> bs;
	bs.reset();
	bool b[size()];
	ProtocolExecution::prot_exec->reveal(b, party, (block *)bits.data(), size());
...
template<>
inline int32_t Integer::reveal<int32_t>(int64_t party) const {
	return reveal<uint32_t>(party);
}

template<>
inline int64_t Integer::reveal<int64_t>(int64_t party) const {
	return reveal<uint64_t>(party);
}


template<>
inline string Integer::reveal<string>(int64_t party) const {
...
template<typename T>
inline void Integer::reveal(T * output, const int64_t party) const {
	bool * b = new bool[size()];
	revealBools(b, party);
