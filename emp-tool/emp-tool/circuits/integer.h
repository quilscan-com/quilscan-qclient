...
class Integer : public Swappable<Integer>, public Comparable<Integer> { public:
	...
	Integer(int length, int64_t input, int64_t party = PUBLIC);

	template<typename T>
		Integer(int length, T * input, int64_t party = PUBLIC);

	template<typename T>
		Integer(T * input, int64_t party = PUBLIC);
	...
	size_t size() const;
	template<typename T>
		T reveal(int64_t party=PUBLIC) const;
	template<typename T>
		void reveal(T * output, const int64_t party=PUBLIC) const;
	...
	void init(bool * b, int len, int64_t party);
	void revealBools(bool *bools, int64_t party=PUBLIC) const;
};
